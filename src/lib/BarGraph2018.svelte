<script>
	...
	let xAxis2018, yAxis2018;

	onMount(() => {
		xAxis2018 = d3.axisBottom(xScale).tickFormat(d3.format('d'));
		yAxis2018 = d3.axisLeft(yScale).ticks(5);
		updateScales(); // Initial update on mount
	});

	// Function to update scales and redraw the axes
	function updateScales() {
		d3.select('.x-axis2018').call(xAxis2018);
		d3.select('.y-axis2018').call(yAxis2018);
	}

	// Reactive statement for handling changes in dataArray
	$: if (dataArray2018.length > 0) {
		xScale.domain(dataArray2018.map(d => d.year));
		yScale.domain([0, d3.max(dataArray2018, d => d.count)]);
		if (xAxis2018 && yAxis2018) { // Check if axes are initialized
			updateScales();
		}
	}
</script>
...
<svg id="barchart-svg" width={width + margin.left + margin.right} height={height + margin.top + margin.bottom}>
	<g transform={`translate(${margin.left},${margin.top})`}>
		{#each dataArray2018 as d, i (d.year)}
			<rect class="bar"
						x={xScale(d.year)}
						y={yScale(d.count)}
		...
		<g transform={`translate(0, ${height})`} class="x-axis2018">
